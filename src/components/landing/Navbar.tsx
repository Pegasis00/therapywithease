    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link to="/signup"><Button variant="hero" size="sm" className="rounded-full px-6">Get Started</Button></Link>
        </div>
        ...
        {open && (
          ...
          >
            <div className="flex flex-col gap-3 p-4">
              ...
              <Link to="/signup" onClick={() => setOpen(false)}><Button variant="hero" className="w-full rounded-full">Get Started</Button></Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
